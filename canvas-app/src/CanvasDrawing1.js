...
function CanvasDrawing1() {
  ...
  const startDrawing = ({ nativeEvent }) => {
    ...
    const shapeIndex = detectShape(x, y);  //get the index position of the the clicked shape
    console.log(shapeIndex);
    
    if (shapeIndex !== -1) {
      setDraggingShapeIndex(shapeIndex);
      const shape = shapes[shapeIndex];  //go to that particular shape in the shapes array
